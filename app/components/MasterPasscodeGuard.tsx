...
export function MasterPasscodeGuard({ isDark, children }: { isDark: boolean; children: React.ReactNode }) {
  ...
  // On server and before mount — always render children to avoid mismatch
  if (!mounted) return <>{children}</>;
  // After mount — check real passcode state
  if (appPasscodeVerified) return <>{children}</>;
      ...
      <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:36,
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:16 }}>
          <div style={{ width:68, height:68, borderRadius:20,
            ...
            boxShadow:"0 8px 32px rgba(245,255,94,0.25)" }}>K</div>
          <div style={{ textAlign:"center" }}>
            <div style={{ fontSize:22, fontWeight:900, color:"#fff", letterSpacing:"-0.03em" }}>Korgon Finance</div>
            <div style={{ fontSize:13, color:"rgba(255,255,255,0.28)", marginTop:5 }}>Enter your passcode to continue</div>
          </div>
        </div>

        <div style={{ display:"flex", gap:10, animation:shake?"shake 0.6s ease":"none" }}>
          {digits.map((d,i) => (
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          {digits.map((_,i) => (
            <div key={i} style={{ width:i<filled?20:6, height:6, borderRadius:99,
        ...
        {error && (
          <div style={{ padding:"8px 20px", borderRadius:99,
